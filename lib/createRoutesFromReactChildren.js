...
function createRouteFromReactElement(element, triggerEvent) {
    ...
    if (type === DefaultRoute) return Route.createDefaultRoute(createRouteOptions(props, triggerEvent));

    if (type === NotFoundRoute) return Route.createNotFoundRoute(createRouteOptions(props, triggerEvent));

    if (type === Redirect) return Route.createRedirect(createRouteOptions(props, triggerEvent));
...
function createRoutesFromReactChildren(children, triggerEvent) {
    ...
    React.Children.forEach(children, function (child) {
        if (child = createRouteFromReactElement(child, triggerEvent)) routes.push(child);
    });
